    ...
    User.verifyAdmin(req.userId).then(
        _ => {
            User.findById(req.body.user, (error, user) => {
                if (error) {
                    res.status(500).send("[GreenhouseAccess::register] error registering access : " + error)
                }else {
                    GreenhouseAccess.create({user:user._id, greenhouse:req.body.greenhouse}).then(
                        success => res.status(200).send({created: true}),
                        error => res.status(500).send("[GreenhouseAccess::register] error registering access : " + error)
                    )
                }
            });
        },
        error => res.status(500).send("[GreenhouseAccess::register] error registering access : " + error ? error : "no admin rights")
    )
...
router.post('/delete', verifyToken, function (req, res) {
    User.verifyAdmin(req.userId).then(
        ...
        error => res.status(500).send("[GreenhouseAccess::delete] error deleting access : " + error ? error : "no admin rights")
    )
